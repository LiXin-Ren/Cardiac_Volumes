...
def enumerate_sax_files(dir, patient_ids=None, filter_slice_type="sax"):
    """
    :param dir: The dir of dicom files
    :param patient_ids:
    :param filter_slice_type:
    :return: DicomWrapper 类数据。可以得到一些属性和图像数据
    """
    
    if not os.path.exists(dir):
        print('not exist')
   
    for root, _, files in os.walk(dir):         #会依次遍历每个文件夹，包括子文件夹
        for file_name in files:                 #IM-4557-0001.dcm
            if file_name.endswith(".dcm"):
                parts = root.split('/')         #'D:/data/kaggel_second annual data science bowl/train/1/study/2ch_21'
                patient_id = parts[-3]
                slice_type = parts[-1]
...
def prepare_cropped_sax_image(sax_image, clahe=True, intermediate_crop=0, rotate=0):
    ...
    if intermediate_crop == 0:
        res = sax_image[settings_unet.CROP_INDENT_Y:settings_unet.CROP_INDENT_Y + settings_unet.TARGET_CROP, settings_unet.CROP_INDENT_X:settings_unet.CROP_INDENT_X + settings_unet.TARGET_CROP]
    else:
        crop_indent_y = settings_unet.CROP_INDENT_Y - ((intermediate_crop - settings_unet.TARGET_CROP) // 2)
        crop_indent_x = settings_unet.CROP_INDENT_X - ((intermediate_crop - settings_unet.TARGET_CROP) // 2)
        res = sax_image[crop_indent_y:crop_indent_y + intermediate_crop, crop_indent_x:crop_indent_x + intermediate_crop]
        res = cv2.resize(res, (settings_unet.TARGET_CROP, settings_unet.TARGET_CROP))
